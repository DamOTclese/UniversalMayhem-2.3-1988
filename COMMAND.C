...
/* **********************************************************************
   ...
   ********************************************************************** */
   ...
   static void if_any_bounce_it(int top_count);
   static void remove_attacked_ship(char this_ship);
   static void counter_assist(short attacking_ship, short m_run_ship);
   static char ship_attack(char the_ship);
...
/* **********************************************************************
   ...
   ********************************************************************** */
   ...
   static char sysop_output[80];
   short last_priority, decoy_xpos, decoy_ypos, decoy_value;
   static short attacked_ships[TOTAL_PLAYERS];

/* **********************************************************************
   * Define the function prototypes that are used only here.            *
   *                                                                    *
   ********************************************************************** */

   static void download_command_file(char *fname);
   static void erase_command_file(char *fname);
...
*/
   ...
   if ((command_file = mayhem_fopen(file_name, "rt", command_file)) == (FILE *)NULL) {
      c_out(WHITE, "Do you want to create a command file for your ship? ");
      timed_input(0);
      ucase(record);
      ...
      if ((command_file = mayhem_fopen(file_name, "wt", command_file)) == (FILE *)NULL) {
         c_out(WHITE, "Unable to create new command file\n\r");
         return;
      }
...
*/
   ...
   if (! newly_created) {
ask_it_again:
      c_out(LIGHTGREEN, "Do you want to (D)ownload your command file:\n\r");
      c_out(LIGHTGREEN, "               (E)rase your command file:\n\r");
      c_out(LIGHTGREEN, "               (M)odify your command file: ");
      timed_input(0);
      ucase(record);
      if (record[0] == 'D') {
         download_command_file(file_name);
         return;
      }
      if (record[0] == 'E') {
         erase_command_file(file_name);
         return;
      }
      if (record[0] != 'M') {
         goto ask_it_again;
...
*/
   ...
   if (decoy_bail == 0L) {
      ...
      c_out(WHITE, "Your bail-out with decoy shuttle class is %d", decoy_bail);
   }
   c_out(WHITE, "\n\rDo you want to change this? ");
   timed_input(0);
   ucase(record);
   if (record[0] == 'Y') {
      c_out(WHITE, "Enter new bail-out with decoy shuttle class: (0 to turn off): ");
      timed_input(0);
...
*/
   ...
   if ((command_file = mayhem_fopen(file_name, "wt", command_file)) == (FILE *)NULL) {
      c_out(WHITE, "I was unable to create the new command file!\n\r");
      suspended_real_time = FALSE;
      return;
   }
...
/* **********************************************************************
   * After the ship data has been read in, go through the names of the	*
   * ship and see if there is a command file associated with it. If	*
   * there is, open it up and scan through it. See if there are any	*
   * 'assist', 'defend', 'attack', run-from', bail-out', and 'decoy'.	*
   *									*
   * If these values are present and describe activities which are to	*
   * take place when this particular player is active, plug the linked	*
   * list structure with the relevant data.				*
   *									*
   ********************************************************************** */
...
void plug_command_files(void)
{
   ...
   for (count = 0; count < players; count++) {
      attacked_ships[count] = (char)NIL;
      if (Good_Hold(count) && count != user_number) {
         STRNCPY(file_name, hold[count]->names, 4);
         if (count == 0) {
            strcpy(file_name, "GP");
         }
         else {
   ...
   if (m_first)
      bounce_run_from_against_standings();

   m_run = m_first;     /* Do this LAST! */
}

/* **********************************************************************
   * Go through the opened command file and see if there are any	*
   * directives for the active ship.					*
   *									*
   * Place the data into a linked list.					*
   * 'assist', 'defend', 'attack', run-from', bail-out', and 'decoy'.	*
   *									*
   ********************************************************************** */
...
void read_command(short count)
{
   ...
   multi.assist = 0;       multi.defend = 0;       multi.attack = 0;
   multi.run_from = 0;     multi.decoy = 0;        multi.bail_1_out = 0;
   multi.bail_2_out = 0;   multi.bail_3_out = 0;   multi.bail_4_out = 0;
   multi.decoy_class = 0;  multi.bail_1_value = 0; multi.phaser = (UL)0;
   multi.bail_2_value = 0; multi.bail_3_value = 0;
   multi.default_fight = 0;
         ...
         if (strlen(record) > 4) {
	       ...
	       active_flag = 1;
               ...
               if (! strncmp(tpoint, "POWER", 5)) {
                  ...
                  multi.bail_1_out = 1;
                  skipspace(tpoint);
                  multi.bail_1_value = atol(tpoint);
                  if (multi.bail_1_value < 1) {
                     multi.bail_1_out = 0;
                  }
               }
               else if (! strncmp(tpoint, "TORP", 4)) {
                  ...
                  multi.bail_2_out = 1;
                  skipspace(tpoint);
                  multi.bail_2_value = atoi(tpoint);
                  if (multi.bail_2_value < 1) {
                     multi.bail_2_out = 0;
                  }
               }
               else if (! strncmp(tpoint, "TIME", 4)) {
                  ...
                  multi.bail_3_out = 1;
                  skipspace(tpoint);
                  multi.bail_3_value = atoi(tpoint);
                  if (multi.bail_3_value < 1) {
                     multi.bail_3_out = 0;
                  }
               }
               else if (! strncmp(tpoint, "DAMAGE", 6)) {
                  ...
                  tpoint += 6;
                  skipspace(tpoint);
                  multi.decoy_class = atoi(tpoint);
                  if (multi.decoy_class < 1) {
                     multi.decoy_class = 0;
                  }
               }
            }
               ...
               tpoint += 8;
               skipspace(tpoint);
               multi.default_fight = atoi(tpoint);
               if (multi.default_fight == 1) {
                  active_flag = 1;
               }
            }
         }
...
/*
   If the active_flag is set, then this enemy ship has something to do with
   the active ship. Because of this, store the information in the structure
   at the end of the linked list. If not, throw the data away.
*/

   if (active_flag == 1) {
      append_linked_list(count);
   }

/*
   Diagnostic: Display the linked list.
*/

/*
   m_test = m_first;
   while (m_test) {
      c_out(LIGHTRED, "TEST: Link %p, next %p\n\r", m_test, m_test->next);
      m_test = m_test->next;
   }
*/
}
...
/* **********************************************************************
   * Take the data in structure 'multi' and append it. Ask for some	*
   * memory for it before we do so, however.				*
   *									*
   ********************************************************************** */

void append_linked_list(short count)
{
   m_point = (struct command_options *)farmalloc(sizeof(struct command_options));

   if (! m_point) {
      c_out(WHITE, "Unable to allocate memory to automate ships->\n\r");
      log_error(1);
      perform_quit(10);
   }

   memory_allocated((UL)sizeof(struct command_options));

   if (m_first == (struct command_options *)NULL) {
      m_first = m_point;
      m_last = m_point;
   }
   else {
   ...
   m_point->next = (struct command_options *)NULL;

   m_point->ship =          count;
   m_point->assist =        multi.assist;
   m_point->defend =        multi.defend;
   m_point->attack =        multi.attack;
   m_point->run_from =      multi.run_from;
   m_point->decoy =         multi.decoy;
   m_point->bail_1_out =    multi.bail_1_out;
   m_point->bail_2_out =    multi.bail_2_out;
   m_point->bail_3_out =    multi.bail_3_out;
   m_point->bail_4_out =    multi.bail_4_out;
   m_point->default_fight = multi.default_fight;
   m_point->decoy_class =   multi.decoy_class;
   m_point->bail_1_value =  multi.bail_1_value;
   m_point->bail_2_value =  multi.bail_2_value;
   m_point->bail_3_value =  multi.bail_3_value;
   m_point->phaser =        multi.phaser;
}

/* **********************************************************************
   * This function strips the leading spaces or tabs from the record	*
   * and then checks to see if the name following, if there is one, is	*
   * the same as the name of the currently active ship.			*
   *									*
   ********************************************************************** */
...
{
   skipspace(tpoint);
   if (! strncmp(tpoint, hold[user_number]->names, 4)) return(TRUE);
   if (! strncmp(tpoint, "ALL", 3)) return(TRUE);
   return(FALSE);
}

/* **********************************************************************
   * Here is the kernal of the psudo-multitasking function. It gets	*
   * called before a command is entered and during inactivity on the	*
   * part of the player. Inactivity will cause the run-time function to	*
   * be called about every three or four seconds.			*
   *									*
   * Depending on processing time, we can add just about anything we	*
   * want to this function and allow it to operate like a mini		*
   * operating system.							*
   *									*
   ********************************************************************** */
...
{
   if (suspended_real_time) {
      return;
   }

   skim_command_file_queue();
   test_morale_level();
}
...
void skim_command_file_queue(void)
{
   ...
   if (Good_Hold(running_ship) && m_run->attack == 1) {
        ...
        for (allies_check = 0; allies_check < 5; allies_check++) {
            if (! strcmp(hold[user_number]->names, enemy->allies[allies_check])) {
                allow_attack = FALSE;
            }
        }
...
/*
   ...
   at the ship the real active ship is killing, then restoreing the
...
*/
   ...
   for (scanning = 0; scanning < ship_count; scanning++) {
      for (scan2 = 0; scan2 < players; scan2++) {
         if (close_ship[scanning] == attacked_ships[scan2]) {
            if (Good_Hold(attacked_ships[scan2])) {
               ...
               c_out(LIGHTRED,
                  "Assisting ship %s is attacking %s. %ld unit hit, %ld remaining\n\r",
...
/* **********************************************************************
   * The assisting ship has destroyed the ship that the active ship was	*
   * attacking. Mark it as being destroyed and display a message.	*
   *									*
   * Don't forget to credit the assisting ship with the kill.		*
   *									*
   ********************************************************************** */
...
static void remove_attacked_ship(char this_ship)
{
   ...
   strcpy(enemy->who_destroyed, hold[m_run->ship]->names);
   write_enemy(this_ship);
   hand_over_planets(this_ship, m_run->ship);
   return_slaver_parts(this_ship, m_run->ship);

   c_out(LIGHTRED, "Assisting ship %s DESTROYED ship %s!\n\r",
      hold[m_run->ship]->names,
      hold[this_ship]->names);

   read_enemy(m_run->ship);
   enemy->total_kills++;
   hold[m_run->ship]->kills++;
   write_enemy(m_run->ship);

   inform_kill(hold[m_run->ship]->names, hold[this_ship]->names, TRUE, 3);

   memory_freed((UL)sizeof(struct holder));
   farfree(hold[this_ship]);
   hold[this_ship] = (struct holder *)NULL;
   perform_stand(TRUE);
}
...
/* **********************************************************************
   * If under attack and bail-out is near, perform decoy operations.	*
   *									*
   ********************************************************************** */
...
void command_decoy(void)
{
   ...
   if (! ship_attack(m_run->ship)) return;

/*
   See if the automated ship has a class shuttle that was specified.
   If not, no shuttle may be used so return.
*/

   if (enemy->ship_shuttle < m_run->decoy_class) {
      return;
   }

   read_enemy(m_run->ship);
   enemy->ship_shuttle -= m_run->decoy_class;
   write_enemy(m_run->ship);
   decoy_xpos = enemy->ship_xpos;
   decoy_ypos = enemy->ship_ypos;
   decoy_value = m_run->ship;
}

/* **********************************************************************
   * If under attack and power is less than bail_1_value...		*
   *									*
   ********************************************************************** */
...
unsigned short bail_1(void)
{
   ...
   if (enemy->ship_power > m_run->bail_1_value) {
      return(FALSE);
   }
...
/* **********************************************************************
   * If under attack and torp is less than bail_2_value...		*
   *									*
   ********************************************************************** */
...
unsigned short bail_2(void)
{
   ...
   if (enemy->ship_torp > m_run->bail_2_value) {
      return(FALSE);
   }
...
/* **********************************************************************
   * If time remaining is less than or equal to bail_3_value, make this	*
   * ship inactive. Also remove it from the linked list.		*
   *									*
   ********************************************************************** */
...
unsigned short bail_3(void)
{
   ...
   if (time_remaining > m_run->bail_3_value) {
      return(FALSE);
   }
...
/* **********************************************************************
   * See if the ship has acquired damage. If so, bail-out.		*
   *									*
   ********************************************************************** */
...
unsigned short bail_4(void)
{
   ...
   if (enemy->ship_hull < 100 && ship_attack(m_run->ship)) {
      the_ship = m_run->ship;
      perform_bail_out();
      write_enemy(the_ship);
      return(TRUE);
...
*/
   ...
   for (wloop = 0; wloop < the_times && bail_out == FALSE; wloop++) {
...
/* **********************************************************************
   * This is the routine which actually severs the two threads to the	*
   * link. It then extracts the object and then sews the two threads	*
...
{
   m_next = m_test->next;       /* m_test is the ship being removed */
   m_run = m_first;             /* Start at the top of the linked list */
   memory_freed((UL)sizeof(struct command_options));
   farfree(m_test);

/*
   If it's the first in the list...
   If the previous is null, then the object we took out's next becomes
   the first. If the object we took out's next was null, then the new
   first object becomes the last as well.
*/

   if (m_previous == (struct command_options *)NULL) {
      m_first = m_next;
      if (m_first->next == (struct command_options *)NULL) {
         m_last = m_next;
      }
      return;
   }
...
/*
   If it's somewhere in the middle or the end...
   The previous link before the one we took out's next is set to the
   nexts of the object we just took out.
*/
...
*/
   ...
   for (stest = 0; stest < players; stest++) {
      if (Good_Hold(stest)) {
	 for (rtest = 0; rtest < 10; rtest++) {
	    if (hold[stest]->xremotes[rtest] == oldx &&
	       ...
	       read_enemy(stest);
               ships->rem_xpos[rtest] = xpos;
               ships->rem_ypos[rtest] = ypos;
	       write_enemy(stest);
	    }
	 }
      }
...
/* **********************************************************************
   * Can't do it. Find out what could not be done and display a message	*
   * to let them know it can't be done.					*
   *									*
   ********************************************************************** */

void cant_do_it(short first_one, short second_one)
{
   c_out(WHITE, "You cant ");

   switch (first_one) {
      case 0: c_out(WHITE, "attack ");
	 break;
      case 1: c_out(WHITE, "defend ");
	 break;
      case 2: c_out(WHITE, "assist against ");
	 break;
      case 3: c_out(WHITE, "run from ");
	 break;
      }

   c_out(WHITE, "a ship while ");

   switch (second_one) {
      case 0: c_out(WHITE, "attacking ");
	 break;
      case 1: c_out(WHITE, "defending ");
	 break;
      case 2: c_out(WHITE, "assisting against ");
	 break;
      case 3: c_out(WHITE, "running from ");
	 break;
      }
...
/* **********************************************************************
   * An addition for version 1.2 was the removal of the run-from anyone	*
   * if a ships standings were in the top three.			*
   *									*
   ********************************************************************** */

void bounce_run_from_against_standings(void)
{
   int sloop, top_count, dloop;
   ...
   for (sloop = 0; sloop < 3; sloop++) {
	 ...
	 if_any_bounce_it(top_count);
      }
   }

/*
   Build the standings file up again.
*/
...
/* **********************************************************************
   * Search through the linked list, (if there is one), and see if the	*
   * ship in the argument has a structure in the linked list that has	*
   * the run-from flag thrown. If so, take that flag out.		*
   *									*
   ********************************************************************** */
...
static void if_any_bounce_it(int top_count)
{
   ...
   while (m_test) {
      if (m_test->ship == top_count && m_test->run_from == 1) {
         remove_link();
	 return;
      }
...
*/
   ...
   c_out(LIGHTRED,
      "%s counter attacked ship %s, %ld units remain in assisting ship\n\r",
...
/* **********************************************************************
   * Does the active ship (or the one we want to know about) have a     *
   * command file that governs it?                                      *
   *                                                                    *
...
char command_file_exist(char for_this_ship)
{
   ...
   if (! Good_Hold(for_this_ship)) return(FALSE);

   strcpy(file_name, hold[for_this_ship]->names);
   strcat(file_name, ".SHP");

   temp_file = (FILE *)NULL;
   temp_file = mayhem_fopen(file_name, "rt", temp_file);

   if (temp_file != (FILE *)NULL) {
      mayhem_fclose(&temp_file, file_name);
      return(TRUE);
   }
...
/* **********************************************************************
   * After waiting for enter to be hit, display the command file for    *
   * this ship.                                                         *
   *                                                                    *
   ********************************************************************** */

static void download_command_file(char *file_name)
{
   char hold_want_color;
          
   c_out(LIGHTGREEN, "Hit [enter] to start receiving your command file:");
   timed_input(0);

   if (command_file == (FILE *)NULL) {
      c_out(LIGHTRED, "*** Mayhem programming error! File not available!\n\r");
      return;
   }

   rewind(command_file);
   hold_want_color = want_color;
   want_color = FALSE;
   c_out(LIGHTGREEN, "\n\r; Command file for ");
   c_out(LIGHTGREEN, hold[user_number]->names);
   c_out(LIGHTGREEN, "\n\r");
   c_out(LIGHTGREEN, "; Date: %s\n\r", the_date);
   ...
   mayhem_fclose(&command_file, file_name);
   want_color = hold_want_color;
}

/* **********************************************************************
   * Allow the removal of the command file. For this to work, however,  *
   * the ships password must be entered.                                *
   *                                                                    *
   ********************************************************************** */

static void erase_command_file(char *file_name)
{
   if (command_file != (FILE *)NULL)
       mayhem_fclose(&command_file, file_name);

   c_out(WHITE, "\n\rTo erase the ships command file, you need to tell me\n\r");
   c_out(WHITE, "your ships password to make sure: ");
   timed_input(1);
   ucase(record);
   if (strcmp(record, ships->ship_pass)) return;
   remove_command_file(hold[user_number]->names, (char *)NULL);
   valid_command_file = FALSE;
   c_out(LIGHTRED, "Your command file has been erased!\n\r");
}

/* **********************************************************************
   * When a ship gets destroyed, we remove the command file automating	*
   * it. This is in case another player creates a ship with the same	*
   * name. If we were to leave it, a command file would exist for the	*
   * player that was not created for it.				*
   *									*
   * Later on in the project, at the version 1.40 version, that is, we  *
   * added the removal of the scout ship files.                         *
   *                                                                    *
   * In version 1.50, when the cops get killed, we create a new command *
   * file for the cops.                                                 *
   *                                                                    *
   ********************************************************************** */
...
{
   char file_name[35], scout_file[35];

   strcpy(file_name, the_name);
   strcat(file_name, ".SHP");
   unlink(file_name);

   strcpy(scout_file, the_name);
   strcat(scout_file, ".SCT");
   unlink(scout_file);

   if (! strcmp(the_name, "<GP>")) {
      create_galactic_police(1);

      if ((command_file = mayhem_fopen("gp.shp", "wt", command_file)) != (FILE *)NULL) {
         if (because_of != (char *)NULL) {
            sprintf(record, "ATTACK %s\n", because_of);
            fputs(record, command_file);
            mayhem_fclose(&command_file, "gp.shp");
         }
      }
   }
}
...
/* **********************************************************************
   * See if this_ship is assisting the current ship. Return TRUE or     *
   * FALSE.                                                             *
   *                                                                    *
   ********************************************************************** */
...
{
    struct command_options *c_test;

    c_test = m_first;

    while (c_test) {
        if (c_test->ship == this_ship) {
            return(c_test->assist == 1 || c_test->defend == 1);
        }
        c_test = c_test->next;
    }
