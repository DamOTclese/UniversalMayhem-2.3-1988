...
void perform_leave(void)
{
   ...
   if (planets.technology == 0) {
	 ...
	 c_out(LIGHTRED, "Shore leave party are all dead! Planet was destroyed!\n\r");
         ships->ship_crew = 40;
	 write_user();
         return;
      }
...
void test_morale_level(void)
{
    ...
    if (ships->ship_morale == 10) {
        ...
        c_out(LIGHTRED, "CREW IS REVOLTING! They demanded shore leave and didn't get it!\n\r");
        c_out(LIGHTRED, "The battle to put down the revolt didn't go so well:\n\r");
        ...
        c_out(LIGHTRED, "   - %ld unit power depleation for force shields to\n\r",
            the_total);

        c_out(LIGHTRED, "     protect engineering and command areas, %ld remain\n\r",
            ships->ship_power);

        ships->ship_shield = 0l;

        c_out(LIGHTRED, "   - Defense shield not repairable. Need a planet for repairs\n\r");
