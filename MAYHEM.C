...
void main(int argc, char *argv[])
{
   ...
   if ((config = mayhem_fopen("SHIP.CFG", "r", config)) == (FILE *)NULL) {
      fputs("File: SHIP.CFG is missing!\n\r", stderr);
      exit(0);
   }
   ...
   if (port_assignment != 0 && port_assignment != 1) {
      fputs("Port assignment in SHIP.CFG is not supported!\n\r", stderr);
      exit(0);
   }
...
/* **********************************************************************
   ...
   ********************************************************************** */
   ...
   if ((updatefile = mayhem_fopen("lastupdt.shp", "r", updatefile)) != (FILE *)NULL) {
      fgets(last_update, 200, updatefile);
      mayhem_fclose(&updatefile);
   }
   else {
      strcpy(last_update, "None");
   }
   ...
   if ((screen = mayhem_fopen("ship.scr", "rb", screen)) != (FILE *)NULL) {
	 ...
	 if (check_for_key()) {
         ...
         fgets(record, 200, screen);
         sprintf(out_buffer, "%s", record);
   ...
   sprintf(out_buffer, "broken alliances.\n\r");
   send_string(out_buffer);

give_menu:
   sprintf(out_buffer, "\n\r\n\r1 ... Run Universal Mayhem\n\r");
   send_string(out_buffer);
   sprintf(out_buffer, "2 ... Acquire Run-Down of the game\n\r");
   send_string(out_buffer);
   sprintf(out_buffer, "3 ... Where can I get the documentation?\n\r");
   send_string(out_buffer);
   sprintf(out_buffer, "4 ... View latest updates (As of %s)\n\r", last_update);
   send_string(out_buffer);
   sprintf(out_buffer, "5 ... Display top ten players ships\n\r");
   send_string(out_buffer);
   sprintf(out_buffer, "6 ... VIEW OTHER SYSTEMS PLAYER STATISTICS!\n\r");
   send_string(out_buffer);
   sprintf(out_buffer, "7 ... Return to FidoNet software\n\r");
   send_string(out_buffer);
...
static void find_docs(void)
{
   ...
   sprintf(out_buffer, "You can find SHIP.DOC in ---> ");
   send_string(out_buffer);
...
static void give_standings(void)
{
   ...
   if ((ship_std = mayhem_fopen("SHIP.STD", "r", ship_std)) == (FILE *)NULL) {
      sprintf(out_buffer, "Standings file: SHIP.STD is not available.\n\r\n\r");
      send_string(out_buffer);
      return;
   }
