...
void remove_destroyed(void)
{
   ...
   for (gone_ship = 1; gone_ship < players; gone_ship++) {
...
void perform_dos(void)
{
   ...
   if (is_redirected == 1) {
      c_out(LIGHTRED, "You can't shell remotely anymore. No redirection!\n\r");
      return;
   }
...
*/
   ...
   enemy->tax_xpos[aloop] = enemy->tax_ypos[aloop] = 0;
   ...
   for (aloop = 0; aloop < OWNABLE; aloop++) {
	 ...
	 for (dloop = 0; dloop < 4; dloop++) {
	    ...
	    if (planets.protected == from_this) {
		  ...
		  for (bloop = 0; bloop < OWNABLE; bloop++) {
